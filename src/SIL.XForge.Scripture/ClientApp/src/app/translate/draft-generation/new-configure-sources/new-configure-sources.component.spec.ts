...
const mockedExternalUrlService = mock(ExternalUrlService);
...
describe('NewConfigureSourcesComponent', () => {
  configureTestingModule(() => ({
    ...
    providers: [
      ...
      { provide: ExternalUrlService, useMock: mockedExternalUrlService }
    ]
  }));
...
class TestEnvironment {
  ...
  constructor(
    ...
    when(mockedI18nService.interpolateVariables(anything(), anything())).thenReturn([
      { text: '' },
      { text: this.activatedProjectDoc.data!.shortName, id: 'currentProjectShortName' },
      { text: '' }
    ]);
    when(mockedI18nService.locale).thenReturn({ helps: '' } as any);
    when(mockedExternalUrlService.configuringSources).thenReturn('https://example.invalid/help');
