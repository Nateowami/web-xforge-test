...
describe('DraftHistoryEntryComponent', () => {
  ...
  describe('entry', () => {
    ...
    it('should show the title of deleted training data files used in the build', fakeAsync(() => {
      // A deleted file (deleted: true) should still appear in historical build info
      const deletedTrainingDataQuery: RealtimeQuery<TrainingDataDoc> = mock(RealtimeQuery<TrainingDataDoc>);
      when(deletedTrainingDataQuery.docs).thenReturn([
        { id: 'doc01', data: { dataId: 'file01', title: 'deleted-file.txt', deleted: true } } as TrainingDataDoc
      ]);
      when(mockedTrainingDataService.queryTrainingDataAsync(anything(), anything())).thenResolve(
        instance(deletedTrainingDataQuery)
      );
