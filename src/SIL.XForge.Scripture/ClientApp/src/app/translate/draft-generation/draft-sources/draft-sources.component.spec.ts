...
// Subject used in tests to emit active training data from the mocked service
const activeTrainingData$: Subject<TrainingData[]> = new Subject<TrainingData[]>();
...
describe('DraftSourcesComponent', () => {
  ...
  describe('save', () => {
    ...
    it('creates training data for added files', fakeAsync(() => {
      ...
      // The service now filters deleted files, so only non-deleted files are emitted
      activeTrainingData$.next([savedFile]);
    ...
    it('deletes training data for removed files', fakeAsync(() => {
      ...
      // The service now filters deleted files, so only non-deleted files are emitted
      activeTrainingData$.next([savedFile1, savedFile2]);
      tick();
    ...
    it('preserves unsaved training file changes when query updates', fakeAsync(() => {
      ...
      // The service now filters deleted files, so only non-deleted files are emitted
      activeTrainingData$.next([initialFile1, initialFile2]);
      tick();
      ...
      // Another client updates the query (remote file added, deleted file excluded by service)
      const remoteFile = { dataId: 'remote_file' } as TrainingData;
      activeTrainingData$.next([initialFile1, initialFile2, remoteFile]);
      tick();
